...
Single-file concatenation of all CIndRA assistant markdowns. Generated on 2026-09-18. Source files live in `assistant/` and `assistant/skills/`; regenerate with `python assistant/build_aggregated_CIndRA.py`.
...
<!-- SOURCE: assistant/skills/product-assembly/SKILL.md -->
...
---
name: data-sources
description: Documents CIndRA data sources including GHCN-Daily, NOAA OISST/ONI/IBTrACS, Copernicus Marine biogeochemical products, and NOAA PIFSC MD50. Use when running a supported download, attribution or unit conversion.
---
...
- **Use in CIndRA**: rainfall, air temperature, tropical cyclones, SST, marine heatwaves and marine biochemistry. Preserve the classification rule implemented by the relevant notebook; do not mix monthly phase labels and annual dominant-phase composites silently.
...
- Always attribute sources in narrative outputs ("Source: GHCN-Daily station <id>", "Source: NOAA OISST v2", "Source: NOAA IBTrACS v04r01", "Source: NOAA ONI").
- Never invent GHCN station IDs; resolve via site config and `GHCN.get_country_code`.
- Always state units: **mm**, **mm/day**, **mm/year**, **°C**, **°C/decade**, **days/year** (rainfall/temperature); **kt**, **cyclones/year**, **ACE/decade** (cyclones).
...
- **`CIndRA_role.md`** — paste the contents into the "Instructions" / system prompt of the assistant. Defines CIndRA's identity, scope (rainfall + air temperature + regional), conventions, data sources, analysis rules, plotting rules, output naming, and error handling for all domains. This is background context CIndRA always has, not something conditionally "activated" — it does not follow the Agent Skills format below.
...
- `notebooks/historical/Regional/` includes multi-station rainfall/temperature, regional NOAA OISST and marine heatwaves, five marine-biochemistry notebooks, and the independent `tropical_cyclones/regional_indicators.ipynb` all-basin IBTrACS workflow.
...
- `data/sites/` — per-site config JSON files. `<country_slug>_<ghcn_station_id>.json` for rainfall/air-temperature (shared between both).
- `outputs/figures/<site_tag>/` and `outputs/tables/<site_tag>/` — per-site figure/table outputs (rainfall, air-temperature; PNG/HTML and CSV/JSON respectively) — see `skills/output-conventions/SKILL.md`.
