...
from site_common import save_table_to_csv as _save_table_to_csv
...
def save_table_to_csv(table_df, output_dir, filename, index=False):
    ...
    Thin wrapper around :func:`site_common.save_table_to_csv` -- sea-level
    tables default to ``index=False`` (unlike the shared default of
    ``index=True``, which rainfall/air-temperature summary tables rely on).
    """
    return _save_table_to_csv(table_df, output_dir, filename, index=index)
...
def load_site_config(config_path):
    ...
    Kept local (rather than imported from :mod:`site_common`) because the
    error message below points at the sea-level site-setup notebook, not
    the rainfall/air-temperature one.
    """
    config_path = Path(config_path)
    if not config_path.exists():
        raise FileNotFoundError(
            f"Site config not found: {config_path}. "
            "Run notebooks/historical/National/sea_level/0_site_setup.ipynb first."
        )
