...
``00_regional_setup.ipynb`` to a Pacific EEZ map: per-station rainfall/
...
``Regional/rainfall/regional_indicators.ipynb`` and
``Regional/air_temperature/regional_indicators.ipynb`` -- it grew out
...
def _station_rainfall_indicators(
    ...
    ``00_regional_setup.ipynb`` applies its completeness filter to the merged
...
def compute_regional_rainfall_indicators(
    ...
    ``stations_data`` is the dictionary produced by ``00_regional_setup.ipynb``
    ...
    observation, independent of ``00_regional_setup.ipynb``'s own (TMIN/TMAX/PRCP
